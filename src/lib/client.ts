 ...
 *
 * Adds:
 *   - `launch(params, opts?)` — `POST /v1/vms` + poll until `status === 'running'`
...
import type { Vm, VmLaunchParams } from '../resources/vms/vms';
...
export class FastvmClient extends Fastvm {
   ...
   *
   * Unlike `client.vms.launch(...)` which returns the initial 201/202 body
   * verbatim, this waits for the VM to actually reach `running` — matching
   * what most callers actually want. Pass `{ wait: false }` to skip polling.
   *
   * @throws {VMLaunchError} VM reached a terminal failure status.
   * @throws {VMNotReadyError} Polling deadline exceeded before `running`.
   */
  async launch(params: VmLaunchParams, opts: LaunchOptions = {}): Promise<Vm> {
    const vm = await this.vms.launch(params);
    if (opts.wait === false || vm.status === RUNNING) return vm;
    return this.waitForVmReady(vm.id, {
      pollIntervalMs: opts.pollIntervalMs ?? DEFAULT_POLL_INTERVAL_MS,
      timeoutMs: opts.timeoutMs ?? DEFAULT_LAUNCH_TIMEOUT_MS,
    });
  }

  /** Poll `GET /v1/vms/{id}` until `status === 'running'` or terminal. */
  async waitForVmReady(vmId: string, opts: WaitForReadyOptions = {}): Promise<Vm> {
    ...
    while (true) {
      const vm = await this.vms.retrieve(vmId);
